...
(function () {
  ...
  KanbanBoard.prototype.updateCharCounter = function () {
    if (!this.descriptionInput || !this.charCounter) return;
    ...
    COLUMNS.forEach(function (column) {
      ...
      columnTasks.forEach(function (task) {
        ...
        if (cardElement) {
          ...
          // زرار "Complete" - يظهر في To Do و In Progress، وينقل التاسك لـ Completed
          ...
          if (todoBtn) {
            todoBtn.addEventListener("click", (event) => {
              event.stopPropagation();
              this.advanceTask(task.id, "todo");
            });
          }
          // زرار "To Do" - يظهر في In Progress و Completed، وينقل التاسك لـ To Do
          const todoBtn = cardElement.querySelector(".todo-task-btn");
          if (todoBtn) {
  ...
  KanbanBoard.prototype.renderTaskCard = function (task) {
    ...
    // نبني زرارات الأكشن حسب الحالة الحالية للتاسك:
    // - To Do: يظهر Start (للانتقال لـ In Progress) و Complete (للانتقال المباشر لـ Completed)
    // - In Progress: يظهر بس Complete
    // - Completed: مفيش زرارات أكشن (التاسك خلص)
    let actionButtonsHtml = "";
    if (task.status === "todo") {
      actionButtonsHtml = `<button class="start-task-btn flex-1 flex items-center justify-center gap-1.5 bg-[#FFFBEB] text-[#FE9A00] text-[13px] font-medium py-2 rounded-lg hover:bg-[#FEF3C7] transition-colors">
            <i class="fa-solid fa-play text-[11px]"></i> Start
        </button>
        <button class="complete-task-btn flex-1 flex items-center justify-center gap-1.5 bg-[#ECFDF5] text-[#00BC7D] text-[13px] font-medium py-2 rounded-lg hover:bg-[#D1FAE5] transition-colors">
            <i class="fa-solid fa-check text-[11px]"></i> Complete
        </button>`;
    } else if (task.status === "in-progress") {
      actionButtonsHtml = `<button class="todo-task-btn flex-1 flex items-center justify-center gap-1.5 bg-[#F1F5F9] text-[#62748E] text-[13px] font-medium py-2 rounded-lg hover:bg-[#E2E8F0] transition-colors">
            <i class="fa-solid fa-rotate-left text-[11px]"></i> To Do
        </button>
        <button class="complete-task-btn flex-1 flex items-center justify-center gap-1.5 bg-[#ECFDF5] text-[#00BC7D] text-[13px] font-medium py-2 rounded-lg hover:bg-[#D1FAE5] transition-colors">
            <i class="fa-solid fa-check text-[11px]"></i> Complete
        </button>`;
      ...
      actionButtonsHtml = `<button class="todo-task-btn flex-1 flex items-center justify-center gap-1.5 bg-[#F1F5F9] text-[#62748E] text-[13px] font-medium py-2 rounded-lg hover:bg-[#E2E8F0] transition-colors">
        ...
        <button class="start-task-btn flex-1 flex items-center justify-center gap-1.5 bg-[#FFFBEB] text-[#FE9A00] text-[13px] font-medium py-2 rounded-lg hover:bg-[#FEF3C7] transition-colors">
            <i class="fa-solid fa-play text-[11px]"></i> Start
        </button>`;
    }
    // كل الحالات الثلاثة دلوقتي عندهم زرارات للتنقل في أي اتجاه
    return (
      '<div id="card-' +
      task.id +
